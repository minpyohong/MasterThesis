...
outputFileName = 'Bloomberg %s-%s-%s %s시 %s분 %s초 result.csv' % (now.year, now.month, now.day, now.hour, now.minute, now.second)
...
def run(**params):
    ...
    for div in div_all :
        ...
        article_datetime =bs_detail_obj.find("div",{"class":"article-content"})
        ...
        bodytext = bs_detail_obj.find("div",{"class":"body-copy-v2 fence-body"})
...
while True :
    breakflag = run(query="BitCoin",fromdate="2018-12-20", todate= '2018-12-27', site= 'https://www.bloomberg.com/' ,articlenum=p)
    if breakflag == "break" :
        break
    p +=10
