...
URL = "https://www.google.com/search?q={query}+after:{fromdate}+before:{todate}+site:{site}&tbm=nws&hl=kr&lr=lang_kr&start={articlenum}"
#URL = "https://www.google.com/search?q={query}+after:{fromdate}+before:{todate}+site:{site}&tbm=nws&hl=en&lr=lang_en&start={articlenum}"
row = []

outputFileName = '(KR)BlockMedia %s-%s-%s %s시 %s분 %s초 result.csv' % (now.year, now.month, now.day, now.hour, now.minute, now.second)
...
def run(**params):
    ...
    for div in div_all :
        ...
        if not bs_detail_obj :
            print("except news")
            continue
        article_datetime = bs_detail_obj.find("div", {"class":"post-meta vcard"})
        article_datetime = bs_detail_obj.find("div", {"class":"post-meta vcard"})
        #print(article_datetime)
        detail_datetime = article_datetime.find('span',{"class":"updated"})

        bodytext = bs_detail_obj.find("div",{"id":"pavo_contents"})
        ...
        head_sentence = bs_detail_obj.find("h1",{"class":"entry-title"})
...
scraping_site = 'https://www.blockmedia.co.kr/'
keyword = "비트코인"
...
while True :
    ...
    breakflag = run(query=keyword, fromdate="2020-5-8", todate="2020-5-11", site=scraping_site, articlenum=p)  # 2020 down2
    if breakflag == "break" :
        break
    p +=10
