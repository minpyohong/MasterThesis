...
while monthDate <= endDate :
    ...
    minus_point = ts_eth.loc[ts_eth['change'] < 0, 'change'].min()
    plus_point = ts_eth.loc[ts_eth['change'] >= 0, 'change'].max()

    bull_ts = ts_eth.loc[ts_eth['change'] == plus_point]   # 최고 상승 지점
    bear_ts = ts_eth.loc[ts_eth['change'] == minus_point]  # 최고 하강 지점

    plt.plot(ts_eth['date'].values, ts_eth['price'].values,'g--')
