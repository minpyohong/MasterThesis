...
while monthDate <= endDate :
    ...
    minus_point = ts_bch.loc[ts_bch['change'] < 0, 'change'].min()
    plus_point = ts_bch.loc[ts_bch['change'] >= 0, 'change'].max()

    bull_ts = ts_bch.loc[ts_bch['change'] == plus_point]   # 최고 상승 지점
    bear_ts = ts_bch.loc[ts_bch['change'] == minus_point]  # 최고 하강 지점
    ...
    plt.xlabel('time')
    plt.ylabel('price')
    #plt.legend(['Bitcoin Cash','bull','bear'])

    plt.legend(['Bitcoin', 'bull', 'bear'])
    plt.show()
