...
f3 = pd.read_csv('dowindex_data.csv',sep=',', thousands=',' , parse_dates=['date'],date_parser=dateparse)
...
f3['change'] = pd.to_numeric(f3['change'].str.replace("%",""))
...
ts3 = f3[['date','price','change']].sort_values(['date'])
...
ts_dow = ts3.loc[(ts3['date'] >= startDate) & (ts3['date'] <= monthDate), :]
...
plt.plot(ts_dow['date'].values, ts_dow['price'].values , 'm--')
